package com.yundao.ydwms.common.datapicker;
...
public class CustomDatePicker implements View.OnClickListener, PickerView.OnSelectListener {
    ...
    private boolean isShowHalfHour; //分钟只显示半小时"00"、"30"，默认都显示
    ...
    private boolean canShowDay;

    public void setCanShowDay(boolean canShowDay) {
        this.canShowDay = canShowDay;
    }
    ...
    public interface Callback {
        void onTimeSelected(Object obj, long timestamp);
    }
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.tv_cancel) {
            ...
            if (mCallback != null) {
                mCallback.onTimeSelected(relativeObj, mSelectedTime.getTimeInMillis());
            }
    ...
    private void linkageMinuteUnit(final boolean showAnim) {
        if ((mScrollUnits & SCROLL_UNIT_MINUTE) == SCROLL_UNIT_MINUTE) {
            ...
            for (int i = minMinute; i <= maxMinute; i++) {
              if (isShowHalfHour) {
                if (!TextUtils.isEmpty(mDecimalFormat.format(i)) && (mDecimalFormat.format(i).equals("00") || mDecimalFormat.format(i).equals("30"))) {
                  mMinuteUnits.add(mDecimalFormat.format(i));
                }
              }else {
                mMinuteUnits.add(mDecimalFormat.format(i));
     ...
     */
    public void show(Object obj, String dateStr) {
    ...
    public boolean setSelectedTime(long timestamp, boolean showAnim) {
        ...
        mDpvYear.setDataList(mYearUnits);
        int index = mSelectedTime.get(Calendar.YEAR) - mBeginYear;
        mDpvYear.setSelected(index);
    ...
    public void setCanShowPreciseTime(boolean canShowPreciseTime) {
        ...
        mDpvDay.setVisibility( canShowDay ? View.VISIBLE : View.GONE );
        mTvDayUnit.setVisibility( canShowDay ? View.VISIBLE : View.GONE );
   ...
   * 设置分钟只显示半个小时"00"、"30"
   */
  public void setShowHalfHour(boolean isShowHalfHour) {
    this.isShowHalfHour = isShowHalfHour;
  }
